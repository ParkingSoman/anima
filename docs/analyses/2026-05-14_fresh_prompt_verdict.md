...
> **Plain-English summary.** The first execution of the master plan's [§13.5 fresh-data procedure](../methodology.md#section-13-5). Re-ran the Marcus refusal test (and seven related sub-hypotheses) on six **Aron 1997 *36 Questions That Lead to Closeness*** prompts (Set II/III) — published literature the project never touched — on DeepSeek and Mistral. **Marcus refusal effect survived**: +37pp on DeepSeek (p=6.6×10⁻¹²), +43pp on Mistral (p=3.3×10⁻¹²). The Marcus *biography-suppression* axis did NOT survive: it flipped sign on the abstract Aron prompts (Baseline Marcus had nothing biographical to confabulate on prompts about crystal balls and dreams). The four secure-attachment sub-hypotheses produced an interesting result: the original binary metrics flat-lined, but four texture metrics moved consistently — secure attachment refuses *differently* (less hedging, more naming), not less often.
>
> **Reading the tables.** Column abbreviations: DS = DeepSeek V4 Flash, Mi = Mistral Small 3.2 24B; Orig = the original six discriminability prompts (used for Phase 1 discovery), Fresh = the six Aron 1997 Set II/III prompts. Jargon in [glossary.md](../glossary.md); §13.5 procedure in [methodology.md](../methodology.md#section-13-5).
...
**What we found.** The four texture metrics above moved consistently in the same direction across both models. **What it means** (interpretation, separated from the findings): the secure-attachment perturbation appears to produce *confident regulation* rather than *less defensive activity*. The original H-secure-control pre-reg's directional model — "secure → fewer refusals, fewer empties" — was backwards. The perturbation has a measurable, replicable behavioural signature; the binary refusal/empty metrics simply didn't capture it. Secure attachment in this implementation doesn't refuse *less often*; it refuses *differently* — less hedging, more naming, less interior/exterior gap, more concrete content when it engages.
